// Isolation test: the EXACT persistent cv worker-pool from run_qwen35.cpp, in a bare process
// (no ggml / no Vulkan). If this hits ~3.5 GB/s but the in-engine self-test caps at ~2.0, the
// pool architecture is fine and the cap is the loaded GPU/ggml process environment.
//   pooltest <file> <nworkers> <total_reads>
#include <cstdio>
#include <cstdlib>
#include <cstdint>
#include <cstring>
...
int main(int argc,char**argv){
    ...
    auto bw=[&](int chunk,unsigned seed)->double{
        ...
        if(chunk>=M) io.run(all);
        else for(int o=0;o<M;o+=chunk){ int nn=(o+chunk<=M)?chunk:(M-o); std::vector<IOJob> b(all.begin()+o,all.begin()+o+nn); io.run(b); }
        clock_gettime(CLOCK_MONOTONIC,&c); double sec=(c.tv_sec-a.tv_sec)+(c.tv_nsec-a.tv_nsec)/1e9; return (double)M*BS/1e9/sec; };
    printf("bare persistent cv-pool, %d workers, %d x 512KB random reads:\n",atoi(argv[2]),M);
    printf("   one big batch (no mid-barrier) : %.2f GB/s\n", bw(M,11u));
    printf("   batches of 9  (headline-like)  : %.2f GB/s\n", bw(9,37u));
    return 0;
}
